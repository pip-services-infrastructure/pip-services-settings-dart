...
class SettingsPersistenceFixture {
  ...
  void testGetAndSet() async {
    var settings = await this._persistence.set(
        null,
        SettingsSectionV1.from('test.1',
    ...
    settings = await this._persistence.set(
        null,
        new SettingsSectionV1.from('test.2',
            ConfigParams.fromTuples(['key1', 'value21', 'key2', 'value22'])));

    expect(settings, isNotNull);
    expect(settings.id, 'test.2');
    expect(settings.parameters.getAsString('key1'), 'value21');

    settings = await this._persistence.getOneById(null, 'test.1');

    expect(settings, isNotNull);
    expect(settings.id, 'test.1');
    expect(settings.parameters.getAsString('key1'), 'value11');

    var page = await this._persistence.getPageByFilter(
  ...
  void testSetParameter() async {
    var settings = await this._persistence.modify(
    ...
    settings = await this._persistence.modify(null, 'test.1',
  ...
  void testIncrementParameter() async {
    var settings = await this
        ._persistence
        .modify(null, 'test.1', null, ConfigParams.fromTuples(['key1', 1]));
    ...
    settings = await this
        ._persistence
        .modify(null, 'test.1', null, ConfigParams.fromTuples(['key1', 2]));
  ...
  void testGetSections() async {
    var settings = await this._persistence.set(
        null,
        new SettingsSectionV1.from('test.1',
            ConfigParams.fromTuples(['key1', 'value11', 'key2', 'value12'])));
    ...
    var page = await this._persistence.getPageByFilter(
        null, new FilterParams(), new PagingParams(0, 10, true));
